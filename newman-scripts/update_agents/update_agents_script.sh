...
for i in $(seq 0 $END); do 
    ...
    ## Backup script
	sshpass -p password ssh -o StrictHostKeyChecking=no xap@$host -C "cp /home/xap/newman-agent/newman-agent.sh /home/xap/newman-agent/newman-agent.sh.backup.${DATE_SUFFIX}"
	success=$?
	echo "ok? $?"
	if [ "$success" != "0" ]; then
