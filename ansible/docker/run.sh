...
docker run --env pass1=$1 --env pass2=$2 --env pass3=$3 --env pass4=$4 --env skipTags=${skipTags} --env module=${module} --env cmdToDo="${cmdToDo}" -it newman/anisble /bin/bash -c "cd /newman && git pull && /newman/ansible/docker/upgrade-all.sh"