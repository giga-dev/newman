...
import Html exposing (Html, text, p, h4)
...
init =
    let
        (agentsAllModel, agentsAllCmd) = AgentsAll.init
        (onDemandAgentsModel, onDemandAgentsCmd) = OnDemandAgents.init
    in
...
update msg model =
    case msg of
        ...
        AgentsAllMsg msg ->
            let
                (newModel, newCmd) = AgentsAll.update msg model.agentsAllModel
            in
                ( { model | agentsAllModel = newModel } , newCmd |> Cmd.map AgentsAllMsg )
        OnDemandAgentsMsg msg ->
            let
                (newModel, newCmd) = OnDemandAgents.update msg model.onDemandAgentsModel
            in
    ...
    Tab.config TabMsg
        ...
        |> Tab.items
            ...
            Tab.item
                ...
                , pane = Tab.pane [] [ AgentsAll.view model.agentsAllModel |> Html.map AgentsAllMsg
                    ]
                }
            ,
            Tab.item
