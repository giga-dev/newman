    ...
    case msg of
                    ...
                    let
                        f elasticGroup =
                            if elasticGroup.id == agentGroupToUpdate.id then
                                let
                                    elasticGroupCapacity = elasticGroup.capacity
                                    newElasticGroupCapacity = { elasticGroupCapacity | target = newCapacity }
                                in
        ...
        UpdateElasticGroupCompleted result ->
             case result of
                Ok elasticGroup ->
                    let
                        l = Debug.log "UpdateElasticGroupCompleted" "group has been updated"
                    in
                        (model, Cmd.none)
                Err err ->
                    let
                        l = Debug.log "UpdateElasticGroupCompleted" <| "group update has failed" ++ (toString err)
                    in
                        (model, Cmd.none)
        Reload ->
...
view model =
    let
        rowOpt elasticGroup =
            if (elasticGroup.capacity.target == elasticGroup.connectedAgents) then Table.rowSuccess else Table.rowDanger
